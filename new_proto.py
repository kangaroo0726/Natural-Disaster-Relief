...
if st.button("Refresh"):
    ...
    # Recalculate remaining_capacity explicitly on session_state df
...
with st.sidebar.expander("Update Your Shelter Info"):
    ...
    if typed_name:
        if typed_name in st.session_state.df["name"].values:
            shelter_row = st.session_state.df[st.session_state.df["name"] == typed_name].iloc[0]
            ...
            if st.button("Update Shelter"):
                # Update individual columns in session_state.df
                st.session_state.df.loc[
                    st.session_state.df["name"] == typed_name, "current_occupancy"] = new_current_occupancy
                st.session_state.df.loc[st.session_state.df["name"] == typed_name, "food"] = new_food
                st.session_state.df.loc[st.session_state.df["name"] == typed_name, "water"] = new_water

                # Explicitly recalc remaining_capacity
                st.session_state.df["remaining_capacity"] = st.session_state.df["capacity"] - st.session_state.df[
                    "current_occupancy"]
